 ...
 */
import { Parser as BinaryParser } from 'binary-parser-encoder';
...
const operation_mode = {
  ...
  fox: 6,
  hound: 7,
  _names: ['NONE', 'NA VHF', 'EU VHF', 'FIELD DAY', 'RTTY RU', 'WW DIGI', 'FOX', 'HOUND'],
  format(code) {
...
const heartbeatParser = new BinaryParser()
  ...
  .choice(null, {
    tag() {
      return payload_format(this.id);
    },
    defaultChoice: new BinaryParser().nest('revision', {
      type: stringParser,
...
// Out/In since v2.0
const clearParser = new BinaryParser().endianess('big');
// since v2.1
// Not sent from WSJTX
// .uint8('window');
...
const wsjtxParser = new BinaryParser()
  ...
  .uint32('schema', {
    ...
    assert: function assert(version) {
      return version >= 0x02;
    },
  })
  .uint32('type')
...
function decode_exchange(message) {
  ...
  if (exchange[2].startsWith('R')) {
    return {
      ...
      dx_call: exchange[0],
      de_call: exchange[1],
      snr: parseInt(exchange[2].slice(1), 10),
    };
  }
